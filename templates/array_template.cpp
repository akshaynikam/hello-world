...
template<typename T, std::size_t SZ>
struct MyClass<T(&)[SZ]> {   // partial specialization for reference to arrays of known bounds
    static void print () { std::cout << "print() for T(&)[" << SZ << "]\n"; }
};
...
template<typename T>
struct MyClass<T(&)[]> {   // partial specialization for reference to arrays of unknown bounds
    static void print () { std::cout << "print() for T(&)[]\n"; }
};
